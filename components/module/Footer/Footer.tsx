...
const FooterLinkMobile: React.FC<FooterLinkMobileMobileType> = ({ className }) => {
  return <div className={`flex gap-3 min-[320px]:gap-[83px] text-[10px] px-4 pt-4 text-gray-11 mb-[29px] ${className}`}>
...
type FooterLinkMainType = {
  className?: string
}
const FooterLinkMain: React.FC<FooterLinkMainType> = ({ className }: FooterLinkMainType) => {
  return <div className={`flex items-start px-4 gap-2.5 min-[317px]:gap-5 min-[381px]:gap-8 ${className}`}>
    <div>
      <p className="text-gray-11 text-sm mb-1 font-medium-shabnam!">خدمات</p>
      <ul className="text-[10px] text-gray-9 flex flex-col gap-1">
        <li>
    ...
    <div>
      <p className="text-sm font-medium-shabnam! text-gray-11 mb-1">
        ارتباط با ما
      </p>
...
const NamadLogo: React.FC<NamadLogoType> = ({ className }: NamadLogoType) => {
  return <div className={`flex justify-end ml-12.5 mt-7  ${className}`}>
    <Image src={'/assets/images/footer/namad.png'} alt="Namad Logo" width={35} height={36} />
  </div>
}
